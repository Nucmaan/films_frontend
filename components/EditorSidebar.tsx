...
export default function EditorSidebar({ isCollapsed = false }: EditorSidebarProps) {
  ...
  const mainMenuItems = [
    { path: "/Editor", icon: Home, label: "Dashboard" },
    { path: "/Editor/My-Tasks", icon: ListChecks, label: "My Tasks" },
    { path: "/Editor/assignedTasks", icon: ClipboardCheck, label: "Assigned Tasks" },
  ];
      ...
      {items.map((item) => {
        ...
        return (
          <Link
            key={item.path}
            href={item.path}
            className={`flex items-center ${isCollapsed ? 'justify-center' : ''} gap-3 ${isCollapsed ? 'px-2' : 'px-4'} py-3 text-sm font-medium rounded-lg transition-all duration-200 ${
              active 
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 ${isCollapsed ? 'w-[74px]' : 'w-64'} bg-white border-r border-gray-100 h-full flex flex-col transition-transform duration-300 ease-in-out ${
        ...
        <div className={`flex-1 overflow-y-auto ${isCollapsed ? 'px-2' : 'px-3'} py-2 space-y-6 no-scrollbar`}>
        ...
        <div className={`${isCollapsed ? 'px-2 flex justify-center' : 'px-3'} py-4 border-t border-gray-100`}>
          <button
            onClick={handleLogout}
            className={`flex items-center ${isCollapsed ? 'justify-center w-12 h-12' : ''} gap-3 ${isCollapsed ? 'p-0' : 'px-4 py-3'} text-sm font-medium rounded-lg transition-all duration-200 ${isCollapsed ? '' : 'w-full'} text-gray-700 hover:bg-[#fff1ec]`}
          >
